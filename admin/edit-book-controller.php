<?php
    ...
    $_POST['book'] = $book_data;

    # Categories
    $sql = "SELECT distinct Cat from tblbooks ";
    $query = $dbh->prepare($sql);
    $query->execute();
    $results = $query->fetchAll(PDO::FETCH_OBJ);
    ...
    if ($query->rowCount() > 0) {
        foreach($results as $row) {
            $_POST['categories'][] = $row->Cat;
        }
    }

    # Authors
    $sql = "SELECT distinct Author from tblbooks ";
    $query = $dbh->prepare($sql);
    $query->execute();
    $results = $query->fetchAll(PDO::FETCH_OBJ);
    $_POST['authors'] = array();
...
<input class="form-control" type="text" name="bookname" value="<?php echo $_POST['book']['BookName'] ?>" required />
</div>
